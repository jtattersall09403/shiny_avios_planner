server <- function(input, output, session) {
  ...
  # Oneworld routes
  get_oneworld_routes <- reactive({
    oneworld_df <- get_avios_airlines()
    ...
    pred_charges <- predict(xgb_model, new_data = oneworld_routes)
  ...
  # Create igraph
  get_route_graph <- reactive({
    
    # Get oneworld airlines and routes
    oneworld_routes <- get_oneworld_routes()
    
    # Create nodes and edges
    edges <- oneworld_routes %>% select(from = source_airport, to = dest_airport)
    flights_graph <- graph_from_edgelist(as.matrix(edges))
    #nodes <- V(flights_graph) %>% names
